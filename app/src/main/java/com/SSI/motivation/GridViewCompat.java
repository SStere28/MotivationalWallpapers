...
public class GridViewCompat extends GridView {
	...
	static {
		try {
			...
			gridView_getChoiceMode = GridView.class.getMethod("getChoiceMode",
					(Class<?>[]) null);
			gridView_getCheckedItemIds = GridView.class.getMethod(
					"getCheckedItemIds", (Class<?>[]) null);
			gridView_isItemChecked = GridView.class.getMethod("isItemChecked",
					int.class);
			gridView_getCheckedItemPosition = GridView.class.getMethod(
					"getCheckedItemPosition", (Class<?>[]) null);
			gridView_getCheckedItemPositions = GridView.class.getMethod(
					"getCheckedItemPositions", (Class<?>[]) null);
			gridView_clearChoices = GridView.class.getMethod("clearChoices",
					(Class<?>[]) null);
			gridView_setItemChecked = GridView.class.getMethod(
					"setItemChecked", int.class, boolean.class);
			gridView_setChoiceMode = GridView.class.getMethod("setChoiceMode",
					int.class);
			gridView_getCheckedItemCount = GridView.class.getMethod(
					"getCheckedItemCount", (Class<?>[]) null);
	...
	private static class LongSparseArray<E> {
		...
		/**
		 * Gets the Object mapped from the specified key, or the specified
		 * Object if no such mapping has been made.
		 */
		...
		/**
		 * Given an index in the range <code>0...size()-1</code>, returns the
		 * key from the <code>index</code>th key-value mapping that this
		 * SparseArray stores.
		 */
		...
		/**
		 * Given an index in the range <code>0...size()-1</code>, returns the
		 * value from the <code>index</code>th key-value mapping that this
		 * SparseArray stores.
		 */
		 ...
		 * specified key, or a negative number if no keys map to the specified
		 * value. Beware that this is a linear search, unlike lookups by key,
		 * and that multiple keys can map to the same value and this will find
		 * only one of them.
		 */
	...
	/**
	 * This api is not implemented yet but can be implemented if you want to set
	 * the multi-selection from the xml file not from the code
	 */
	...
	public void setAdapter(ListAdapter adapter) {
		...
		super.setAdapter(adapter);
		if (adapter != null) {
			if (mChoiceModeC != AbsListView.CHOICE_MODE_NONE
					&& getAdapter().hasStableIds() && mCheckedIdStatesC == null) {
	...
	public long[] getCheckedItemIds() {
		...
		// Code copied from Android source
		if (mChoiceModeC == AbsListView.CHOICE_MODE_NONE
				|| mCheckedIdStatesC == null || getAdapter() == null) {
			return new long[0];
		}
	...
	public boolean isItemChecked(int position) {
		...
		// Code copied from Android source
		if (mChoiceModeC != AbsListView.CHOICE_MODE_NONE && mCheckStatesC != null) {
	...
	public int getCheckedItemPosition() {
		...
		// Code copied from Android source
		if (mChoiceModeC == AbsListView.CHOICE_MODE_SINGLE
				&& mCheckStatesC != null && mCheckStatesC.size() == 1) {
	...
	public void setItemChecked(int position, boolean value) {
		...
		if (mChoiceModeC == AbsListView.CHOICE_MODE_MULTIPLE) {
			...
			if (mCheckedIdStatesC != null && getAdapter().hasStableIds()) {
				if (value) {
					mCheckedIdStatesC.put(getAdapter().getItemId(position),
							position);
				} else {
			...
			boolean updateIds = mCheckedIdStatesC != null
	...
	public boolean performItemClick(View view, int position, long id) {
		...
		if (mChoiceModeC != AbsListView.CHOICE_MODE_NONE) {
			...
			if (mChoiceModeC == AbsListView.CHOICE_MODE_MULTIPLE) {
				...
				if (mCheckedIdStatesC != null && getAdapter().hasStableIds()) {
					if (newValue) {
						...
						mCheckedIdStatesC.delete(getAdapter().getItemId(
								position));
					}
				}
				if (newValue) {
			...
			} else if (mChoiceModeC == AbsListView.CHOICE_MODE_SINGLE) {
				boolean newValue = !mCheckStatesC.get(position, false);
				if (newValue) {
					mCheckStatesC.clear();
					mCheckStatesC.put(position, true);
					...
					mCheckedItemCountC = 1;
				} else if (mCheckStatesC.size() == 0
						|| !mCheckStatesC.valueAt(0)) {
					mCheckedItemCountC = 0;
				}
			}
	...
	public void setChoiceMode(int choiceMode) {
		...
		if (mChoiceModeC != AbsListView.CHOICE_MODE_NONE) {
			...
			if (mCheckedIdStatesC == null && getAdapter() != null
					&& getAdapter().hasStableIds()) {
				mCheckedIdStatesC = new LongSparseArray<Integer>();
			}
		}
	...
	public Parcelable onSaveInstanceState() {
		...
		if (mCheckedIdStatesC != null) {
			...
			for (int i = 0; i < count; i++) {
				idState.put(mCheckedIdStatesC.keyAt(i),
						mCheckedIdStatesC.valueAt(i));
			}
